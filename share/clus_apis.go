...
type CLUSGroup struct {
	...
	EbpfTls         bool                `json:"ebpf_tls,omitempty"`
}
...
// ebpf
const CLUSEbpfStore string = "ebpf/"
const CLUSEbpfTlsStore string = CLUSEbpfStore + "tls/"

type CLUSEbpfTls struct {
	Name      string    `json:"name"`
	MemberIds []string  `json:"member_ids"`
	Status    bool      `json:"status"`
}

func CLUSEbpfTlsKey(name string) string {
	return fmt.Sprintf("%s%s", CLUSEbpfTlsStore, name)
}

func CLUSEbpfTlsKey2Name(key string) string {
	return keyLastToken(key)
}
